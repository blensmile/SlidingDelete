package blen.slidingdelete.Explosion;
...
import blen.slidingdelete.Explosion.factory.ParticleFactory;
...
public class ExplosionField extends View{
    ...
    private void init(ParticleFactory particleFactory) {
        ...
        attach2Activity((Activity) context);//getContext());
    }
    ...
    private void explode(final View view,Rect rect) {
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                //使用回调
                if(mExpEnd != null)
                    mExpEnd.onExplosionAnimationEnd();
            }
        });
        animator.start();
    }
    ...
    //回调
    private OnExplosionAnimationEnd mExpEnd;
    //创对象时候调用
    public void setOnExplosionAnimationEnd(OnExplosionAnimationEnd exp){
        mExpEnd = exp;
    }

    public interface  OnExplosionAnimationEnd{
     ...
     */
    //TODO 其实这里可以改下,把ViewGroup的整体作为一个选项
